...
rbs_facet_data <- tidyr::pivot_longer(data = rbs_grid, cols = as.character(2009:2018), names_to = "year")

rbs_facet_plot <- ggplot(data = rbs_facet_data, aes(fill = value, col=value))+
  geom_sf(na.rm=T)+
  scale_fill_viridis_c(option="viridis",na.value = NA, name = "RBS",direction = -1)+
  scale_color_viridis_c(option="viridis",na.value = NA, name = "RBS",direction = -1)+
  geom_sf(data=land,col=NA,fill="grey")+
  ...
  scale_x_continuous(breaks=coordxmap)+
  scale_y_continuous(breaks=coordymap,expand=c(0,0))+
  coord_sf(xlim=c(coordslim[1], coordslim[2]),
           ylim=c(coordslim[3], coordslim[4]))+
  ylab("Latitude")+
  xlab("Longitude")

rbs_facet_plot

ggsave("inst/extdata/wp4/NS_rbs.jpeg",dpi = 300, height =4.5,width =  8, units = "cm")
